<script lang="ts">
	...
	const chosenDistrict = $derived(district || DISTRICTS.find((d) => search.includes(d)) || '')
	const selectedFee = $derived(chosenDistrict ? DISTRICT_FEES[chosenDistrict] : undefined)
	...
	function saveCurrent() {
		...
		savedAddresses = [
			...savedAddresses,
			{ icon: savedAddresses.length % 2 === 0 ? 'home' : 'office', labelKh: chosenDistrict || 'ទីតាំង', address: loc }
		]
		persist()
	}
	...
	function currentLocation() {
		...
		navigator.geolocation.getCurrentPosition(
			(pos) => {
				...
				search = `Current location (${latitude.toFixed(5)}, ${longitude.toFixed(5)})`
				locating = false
			},
			(err) => {
	...
	function confirm() {
		...
		if (!loc.trim()) return
		booking.update((s) => ({ ...s, location: loc.trim(), district: chosenDistrict }))
		goto('/schedule')
	}
</script>
...
<div class="space-y-6">
	<!-- Map + panel layout -->
	<div class="grid grid-cols-1 lg:grid-cols-[1fr_380px] rounded-2xl overflow-hidden border border-border min-h-[520px]">
		<!-- Left: Map -->
		<div class="relative bg-[#4fa990] min-h-[300px] lg:min-h-full overflow-hidden">
			...
			<svg class="absolute inset-0 w-full h-full" viewBox="0 0 800 600" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg">
				...
				<rect x="200" y="80" width="100" height="60" rx="4" fill="#45998230" />
				<rect x="320" y="50" width="140" height="90" rx="4" fill="#45998225" />
				<rect x="500" y="60" width="110" height="70" rx="4" fill="#45998230" />
				<rect x="650" y="80" width="100" height="80" rx="4" fill="#45998225" />
				<rect x="60" y="200" width="100" height="120" rx="4" fill="#45998225" />
				<rect x="340" y="180" width="120" height="100" rx="4" fill="#45998230" />
				<rect x="520" y="190" width="130" height="90" rx="4" fill="#45998225" />
				<rect x="680" y="200" width="90" height="110" rx="4" fill="#45998230" />
				<rect x="80" y="380" width="110" height="90" rx="4" fill="#45998225" />
				<rect x="240" y="360" width="130" height="100" rx="4" fill="#45998230" />
				<rect x="500" y="370" width="120" height="100" rx="4" fill="#45998225" />
				<rect x="650" y="390" width="110" height="90" rx="4" fill="#45998230" />
				<!-- Water body -->
				...
				<g transform="translate(410,460)">
					...
					<circle cx="0" cy="0" r="16" fill="#1e6b5a" stroke="white" stroke-width="2.5"/>
					<circle cx="0" cy="0" r="6" fill="white"/>
				</g>
			</svg>

			<!-- Current Location button overlay -->
			<button onclick={currentLocation} disabled={locating} class="absolute top-4 left-4 flex items-center gap-2 bg-white rounded-xl px-3 py-2 shadow-md text-sm font-medium text-text hover:shadow-lg disabled:opacity-70 transition-shadow cursor-pointer">
				<svg class="w-4 h-4 text-primary {locating ? 'animate-spin' : ''}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
					{#if locating}
				...
				ទីតាំបច្ចុប្បន
				<span class="text-text-muted text-xs">{locating ? 'Locating…' : 'Current Location'}</span>
			</button>
			{#if locateError}
				<div class="absolute bottom-4 left-4 right-4 bg-red-50 border border-red-200 text-red-700 text-xs rounded-lg px-3 py-2">{locateError}</div>
			{/if}
		</div>
		...
		<div class="bg-white p-6 flex flex-col gap-5 overflow-y-auto">
			...
			<div>
				<div class="flex items-center justify-between mb-1.5">
					<span class="text-xs font-medium text-text">ស្វែងរកទីតាំង ឬ អាសយដ្ឋាន</span>
					<span class="text-xs text-text-muted">Search Location or Address</span>
				</div>
				<div class="flex items-center gap-2 border border-border rounded-xl px-3 py-2.5 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 transition-all bg-gray-50">
			...
			<div>
				...
				<div class="relative">
					...
					<svg class="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-text-muted pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
						<path d="m6 9 6 6 6-6"/>
					</svg>
				</div>
				{#if selectedFee !== undefined}
					<p class="text-xs text-text-muted mt-1.5">ថ្លៃធ្វើដំណើរ / Travel fee for {chosenDistrict}: <span class="font-semibold text-primary">${selectedFee.toFixed(2)}</span></p>
				{/if}
			</div>
			...
			<div>
				<div class="flex items-center justify-between mb-1.5">
					...
					<span class="text-xs text-text-muted">Landmark Notes (e.g. Near Cafe)</span>
				</div>
				<textarea
					bind:value={landmark}
					rows={3}
			...
			<div>
				...
				<div class="space-y-2">
					{#each savedAddresses as addr, i}
						<div class="w-full flex items-center gap-3 p-3 rounded-xl border border-border bg-gray-50 hover:border-primary/40 hover:bg-primary/5 transition-all group">
							<button onclick={() => useSaved(addr)} class="flex items-center gap-3 flex-1 min-w-0 text-left cursor-pointer">
								...
								<div class="min-w-0">
									...
									<p class="text-xs text-text-muted truncate">{addr.address}</p>
								</div>
							</button>
							<button
					...
					{:else}
						<p class="text-xs text-text-muted italic py-2">No saved addresses yet.</p>
					{/each}
				</div>
			</div>
